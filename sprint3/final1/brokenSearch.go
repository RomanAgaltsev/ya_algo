...
Отчеты:
- Ревью 1 (два рекурсивных бинарных поиска) - https://contest.yandex.ru/contest/23815/run-report/112512062/
- Ревью 2 (один итеративный бинарный поиск) - https://contest.yandex.ru/contest/23815/run-report/112861020/
...
Данное решение не прошло ревью, вернулось с замечаниями - переписать на одну функцию поиска и память O(1).

Память O(1) при бинарном поиске возможна только в том случае, если поиск реализован без рекурсии, то есть итеративно.
Поэтому, удалил из кода функции getBoundaries и binarySearch.
brokenSearch переписал на обычный цикл с двумя указателями без использования рекурсии.

Логика brokenSearch следующая:
- Используем указатели left и right, которыми будем "ходить" по массиву. Сначала они равны первому и последнему индексу массива;
- Пока указатели не сошлись, выполняем бинарный поиск в массиве - вычислем указатель на середину mid и т.д.;
- В принципе, это обычный бинарный поиск, только реализована специфичная логика определения половины массива, в которой находится искомый элемент;
	- Сначала проверяем левую половину на наличие границ изначального массива;
	- Если границы изначального массива в левой половине нет (они в правой) и искомое значение в этой половине, двигаем right на середину mid-1;
	- Иначе двигаем left на середину mid+1 - если левая половина отсортирована и между её границ искомого значения нет, то оно в правой половине;
	- Если же границы изначального массива в левой половине, то всё наоборот, только с правой половиной.

Хотя логика brokenSearch усложнилась, в целом решение стало проще, так как избавилось от двух функций и рекурсии.

-- ДОКАЗАТЕЛЬСТВО КОРРЕКТНОСТИ --
Доказательство корректности для нового решения не изменилось. Тесты пройдены, бинарный поиск работает за O(log n).
...
В решении используется только бинарный поиск, который работает за O(log n).
...
В итоге, имеем O(n) для входящих данных.
Дополнительная память O(1) - можно сказать, что почти не используется.
...
После реализации нового решения на одном итеративном бинарном поиске снова сделал замеры (несколько раз).
...
Невооруженным взглядом видно, что время выполнения одного поиска в слайсах всех размеров значительно сократилось.

Если же говорить про замеры на Контесте, то:
- Время сократилось с 143ms до 87ms;
- Память с 2.18Mb до 2.13Mb.
*/
