...
Отчеты:
- Ревью 1 - https://contest.yandex.ru/contest/24414/run-report/113449962/ - 427ms/48.09Mb
- Ревью 2 - https://contest.yandex.ru/contest/24414/run-report/113592734/ - 424ms/49.12Mb
...
Решение вернулось с ревью с замечанием - изменить findNode, чтобы код поиска элемента был в одном месте.

При удалении элемента связного списка, требуется найти предыдущий удаляемому элемент, чтобы подменить ссылки.
Решил, что просто в findNode добавлю пременную для предыдущего элемента и буду возвращать ее вторым результатом.
Возможно, можно было бы реализовать двусвязный список и сразу иметь указатель на prev, но изменения в решении были не косметические.
Да и памяти, скорее всего, потребовалось бы больше.

Поэтому выполнил совсем небольшие доработки решения:
- Функция findNode возвращает предыдущий искомому элемент;
- Там, где предыдущий элемент не нужен, при вызове findNode он записывается в _;
- Код deleteNode получился намного проще и нагляднее - просто вызываем findNode, если предыдущий элемент (prev) не nil, подменяем указатели.
...
//	Возвращает два указателя - на найденный элемент или nil, а также на предыдущий элемент или nil
func (ht *HashTable) findNode(n *node, key int) (*node, *node) {
	// Переменная для предыдущего элемента относительно искомого
	var p *node
	// Обходим связный список, пока не дошли до конца или не нашли нужный элемент
	for n != nil {
		// Если нашли элемент списка с переданным ключом
		if n.key == key {
			// Возвращаем этот элемент и предыдущий от него
			return n, p
		}
...
func (ht *HashTable) put(key, value int) {
	...
	} else if nodeByKey, _ := ht.findNode(node, key); nodeByKey != nil {
...
func (ht *HashTable) get(key int) (int, error) {
	...
	node, _ := ht.findNode(ht.table[i], key)
	// Проверяем, что нашли
	if node != nil {
