...
- Ревью 2 - https://contest.yandex.ru/contest/22781/run-report/111691828/
...
Слайс (массив) будто бы становится бесконечными, а его индексы как-будто "зацикливаются".
...
const (
	// Константы шагов указаталей, чтобы избавиться от магических чисел в коде
	stepRight = 1 // Шаг вправо
	stepLeft = -1 // Шаг влево
)
...
// calculatePosition - вычисляет новое положение указателя по переданным параметрам:
...
func executeCommand(deque *Deque, command Command) string {
	...
	// По переданному имени команды вызываем метод дека
	switch command.name {
	case "push_back":
		// Добавление в хвост
