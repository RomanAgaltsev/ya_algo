...
Обойдите с помощью DFS все вершины, достижимые из заданной вершины s, и выведите их в порядке обхода, если начинать обход из s.
...
func DFS(vertex int) {
    ...
    for len(stack) > 0 {
        ...
        if colors[v] == white {
            ...
            for i := len(adjacencyList[v]) - 1; i >= 0; i-- {
                if colors[adjacencyList[v][i]] == white {
                    stack = append(stack, adjacencyList[v][i])
                }
            }
        }
    }
