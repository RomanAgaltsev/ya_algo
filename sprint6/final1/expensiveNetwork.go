...
Кроме этого, после выполнения алгоритма, еще проверются посещенные вершины - это O(V) в худшем случае.

Получается, общая оценка будет O(E*log(V) + V)
...
func main() {
    ...
    // Создаем новый граф
    g := &Graph{
        n:   n,                   // Количество вершин заполняем в графе как есть
        adj: make([][]Edge, 2*m), // Список связности создаем на двойное количество ребер, т.к. граф неоритентированный - можем переходить по ребру в обе стороны
    }
